...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mColumnChartView = (ColumnChartView) findViewById(R.id.chartView);
        mColumnChartView.setOnValueTouchListener(new ValueTouchListener());
        mColumnChartView.setHorizontalScrollBarEnabled(false);
        mColumnChartView.setVerticalScrollBarEnabled(false);
    ...
    private void generateDefaultData() {
        ...
        for (int i = 0; i < numColumns; ++i) {
            ...
            for (int j = 0; j < numSubcolumns; ++j) {
                SubcolumnValue value = new SubcolumnValue((float) Math.random() * 50f + 5, 0xeeffffff);//第一个值是数值(值>0 方向朝上，值<0，方向朝下)，第二个值是颜色
...
//            column.setHasLabels(hasLabels);//设置是否显示每个柱状图的高度，
            column.setHasLabelsOnlyForSelected(true);//点击的时候是否显示柱状图的高度，和setHasLabels()和用的时候，setHasLabels()失效
            columns.add(column);
        }
        mColumnChartData = new ColumnChartData(columns);//表格的数据实例
        mColumnChartData.setFillRatio(0.5F); //设置柱子宽度
        if (hasAxes) {
            Axis axisX = new Axis();
    ...
    private class ValueTouchListener implements ColumnChartOnValueSelectListener {
        ...
        public void onValueSelected(int columnIndex, int subcolumnIndex, SubcolumnValue value) {
            List<Column> columns = mColumnChartData.getColumns();
            for(Column column : columns) {
                column.getValues().get(0).setColor(0xffff0000);
            }
